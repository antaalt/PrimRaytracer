...
void Application::run(raycore::Scene &scene, raycore::Camera &camera, raycore::tracer::Tracer &tracer)
{
	raycore::StopWatch<> stopWatch;

	m_renderer = new raycore::Renderer(&tracer, m_width, m_height);
	scene.build();
	// LOOP
	do
	{
...
bool Application::inputs(raycore::Camera &camera, const Inputs &inputs)
{
