...
BVHNode::BVHNode(const geometry::point3f & min, const geometry::point3f & max) : BoundingBox(min, max), hitableCount(0)
{
}
BVHNode::~BVHNode()
{
...
unsigned int BVHNode::init(const std::vector<const HitableBounded*>& hitableParents, unsigned int depth)
{
	...
	geometry::point3f centroid[CHILD_COUNT] = {
	...
	do {
		...
		for (const HitableBounded* hitableParent : hitableParents)
		{
			...
			const geometry::point3f c = bbox.center();
			ASSERT(contain(c), "Should be inside");
			const float dist[2] = {
				geometry::point3f::distance(c, centroid[0]),
				geometry::point3f::distance(c, centroid[1])
			};
			if (dist[0] > dist[1])
